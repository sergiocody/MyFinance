...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (session.accounts.length === 1) {
      // Single account: auto-link it directly to the account the user reconnected.
    ...
    // Multiple accounts: let the user pick which bank account to link on the selection page, so
    // each MyFinance account maps to exactly the bank account the user chooses. We deliberately
    // do NOT auto-match across accounts (by IBAN/currency/position) — that previously cross-linked
    // Revolut EUR/GBP and mixed their balances.
    await dbClient.rpc("update_bank_connection_session", {
      p_connection_id: connection.id,
